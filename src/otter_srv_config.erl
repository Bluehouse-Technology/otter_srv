...
list() ->
    application:get_all_env(otter_srv).

read(Key) ->
    application:get_env(otter_srv, Key).

read(Key, Default) ->
    application:get_env(otter_srv, Key, Default).
...
write(Key, Value) ->
    application:set_env(otter_srv, Key, Value).
